...
def debug_state(title, G, node_from, dist_to):
    ...
    tbl = DataTable([6] + [6]*len(labels), ['.'] + labels, output=True)
    tbl.format('.','s')
    for f in labels:
        tbl.format(f, 's')
    for u in labels:
    ...
    tbl_dist_to = DataTable([6] + [6]*len(labels), ['.'] + labels, output=True, decimals=1)
