...
def extract(s, start):
    ...
    raise ValueError('ill-formed string:' + s)
...
def recreate_tree(expr):
    ...
    if expr[0] == '(' and expr[-1] == ')' and not '(' in expr[1:-1]:
        ...
        node = BinaryNode(elements[0])
        node.left = BinaryNode(elements[1]) if elements[1] else None
        node.right = BinaryNode(elements[2]) if elements[2] else None
        return node
    ...
    if expr[comma1+1] == '(':
        ...
        left = recreate_tree(subgroup)
        comma2 = comma1+len(subgroup)+1
        if expr[comma2+1] == '(':
            subgroup = extract(expr,comma2+1)
            right = recreate_tree(subgroup)
        else:
            right = BinaryNode(expr[comma2+1:-1])
    else:
